...
@router.get("")
async def list_api_keys(key_doc: dict = Depends(validate_api_key)):
    """List all API keys (without raw key values). Scoped to caller."""
    keys_collection = mongo.get_keys_collection()
    # Only return the key that the caller used to authenticate
    cursor = keys_collection.find({"_id": key_doc["_id"], "is_active": True})
    keys = []
